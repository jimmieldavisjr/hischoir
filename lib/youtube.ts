...
export async function seedPlaylistSnapshot() {
  const countResult = await getDatabase().query<{ count: number }>(
...
export async function syncYouTubePlaylist() {
  ...
  if (!apiKey) {
    ...
    await database.query("UPDATE sync_state SET last_error = $1 WHERE source = 'youtube'", [message]);
    ...
    await withTransaction(async (client) => {
      ...
      for (const item of items) {
        ...
        const thumbnail =
          ...
          thumbnails.high?.url ??
          `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
        const available = item.status?.privacyStatus !== "private";
        await client.query(
          `INSERT INTO songs
